...
(* The intern_string and extern_atom functions are available from the
  Coq world, where they are axiomatized as a total, bijective mapping
  (see ident_of_coqstring and coqstring_of_ident below). To make sure
  this is sound, their side-effects must not be observable: both may
  insert new entries in the hash tables, but as far as the Coq code
  knows the mapping existed all along and we just "discovered" it.

  This means we need to be able to generate new atoms (next_atom), but
  also fresh dummy strings that may be used in case a yet-undefined atom
  is passed to extern_atom (fresh_temp). Those most likely correspond to
  the names of temporaries generated by the SimplExpr pass, hence the
  naming pattern. *)
