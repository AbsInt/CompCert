...
- (* int *)
  ...
  eapply IHtyl; eauto.
  subst. split; [lia | auto with divide].
  eapply Y; eauto. lia.
- (* float *)
  destruct (zlt fr 8); destruct H.
  subst. apply freg_param_caller_save.
  eapply IHtyl; eauto.
  subst. split. apply Z.le_ge. apply align_le. lia. auto with divide.
...
- (* long *)
  ...
  eapply IHtyl; eauto.
  destruct H. subst p. split; destruct Archi.big_endian; (split; [ lia | auto with divide ]).
...
- (* float *)
  ...
  split; [extlia|eauto with divide].
  eapply Y. eapply IHtyl; eauto. lia.
- (* long *)
  set (ofs' := align ofs 2) in *.
  assert (ofs <= ofs') by (apply align_le; lia).
  ...
  split; destruct Archi.big_endian; (split; [extlia|eauto with divide]).
  eapply Y. eapply IHtyl; eauto. lia.
- (* single *)
  destruct H.
  destruct (zlt ofs 0); subst p.
