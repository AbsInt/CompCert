...
Fixpoint constval (ce: composite_env) (a: expr) : res val :=
  ...
  | Evar x ty =>
      OK(Vptr (Ident.to_positive x) Ptrofs.zero)
  | Ederef r ty =>
      constval ce r
  | Efield l f ty =>
      match typeof l with
...
Definition transl_init_single (ce: composite_env) (ty: type) (a: expr) : res init_data :=
  ...
  | Vptr id ofs, Tint I32 sg _ => assertion (negb Archi.ptr64); OK(Init_addrof (Ident.of_positive id) ofs)
  | Vptr id ofs, Tlong _ _ => assertion (Archi.ptr64); OK(Init_addrof (Ident.of_positive id) ofs)
  | Vptr id ofs, Tpointer _ _ => OK(Init_addrof (Ident.of_positive id) ofs)
