...
Definition transf_function' (f: function) (dm: defmap) (approxs: PMap.t numbering) : function :=
  mkfunction
    f.(fn_sig)
    f.(fn_params)
    f.(fn_stacksize)
...
Inductive match_stackframes: list stackframe -> list stackframe -> Prop :=
  ...
  | match_stackframes_cons:
    ...
    match_stackframes
      ...
      (Stackframe res (transf_function' f (prog_defmap cu) approx) sp pc rs' :: s').
...
Inductive match_states: state -> state -> Prop :=
  | match_states_intro:
      ...
      match_states (State s f sp pc rs m)
                   (State s' (transf_function' f (prog_defmap cu) approx) sp pc rs' m')
  | match_states_call:
...
Ltac TransfInstr :=
  ...
  | f: function, approx: PMap.t numbering,
    ...
    |- _ =>
      cut ((transf_function' f (prog_defmap cu) approx).(fn_code)!pc = Some(transf_instr (prog_defmap cu) approx!!pc instr));
      [ simpl transf_instr
...
* (* turned into a move *)
  ...
  hnf in H9. rewrite E2 in H9. inv H9.
  assert (res0 = v) by congruence. subst res0.
  split; auto.
  apply eventually_later. intros rr F; inv F.
  intros t s2 S2. inv S2.
  split; auto. apply eventually_now.
  econstructor; eauto.
  eapply analysis_correct_1; eauto. simpl; auto.
...
** (* the builtin function succeeds *)
  ...
  assert (res0 = vres) by congruence. subst res0.
  split; auto.
  apply eventually_later. intros rr F; inv F.
  intros t s2 S2. inv S2.
  split; auto. apply eventually_now.
...
- (* Ibuiltin *)
  ...
  intros (v' & m1' & P & Q & R & S).
  assert (DEFAULT:
    (fn_code (transf_function' f (prog_defmap cu) approx)) ! pc = Some(Ibuiltin ef args res pc') ->
    exists s2', step tge (State s' (transf_function' f (prog_defmap cu) approx) sp pc rs' m'0) t s2'
