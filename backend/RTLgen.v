...
Fixpoint reserve_labels (s: stmt) (ms: labelmap)
                        {struct s} : mon labelmap :=
  match s with
  | Sseq s1 s2 | Sifthenelse _ s1 s2 =>
    do lm <- reserve_labels s2 ms;
    reserve_labels s1 lm
  | Sloop s1 | Sblock s1 => reserve_labels s1 ms
  | Slabel lbl s1 =>
    do lm <- reserve_labels s1 ms;
    alloc_label lbl lm
  | _ => ret ms
  end.
