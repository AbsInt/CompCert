...
(* Bounding the recursion depth of size_stmt, so that it runs in constant time.
   Statements too deep are given a constant, large size. *)

let max_stmt_depth = 8
let default_stmt_size = 40

let rec size_stmt d s =
  if d <= 0 then default_stmt_size else
  match s with
  | Sskip -> 0
  ...
  | Sseq(s1, s2) -> size_stmt (d-1) s1 + size_stmt (d-1) s2
  | Sifthenelse(ce, s1, s2) ->
      size_condexpr ce + max (size_stmt (d-1) s1) (size_stmt (d-1) s2)
  | Sloop s -> 1 + 4 * size_stmt (d-1) s
  | Sblock s -> size_stmt d s
  | Sexit n -> 1
  | Sswitch xe -> size_exitexpr xe
  | Sreturn None -> 2
  | Sreturn (Some arg) -> 2 + size_expr arg
  | Slabel(lbl, s) -> size_stmt d s
  | Sgoto lbl -> 1

let size_stmt s = size_stmt max_stmt_depth s
